...
(defn- parts->messages
  ...
  Tool outputs keep the original tool call id from the upstream part so that
  multi-turn tool conversations remain valid for any OpenAI-compatible provider."
  [system-msg input]
  (let [messages (->> input
                      (map (fn [part]
                             (case (:type part)
                               ...
                               :tool-output {:role "tool"
                                             :tool_call_id (or (:id part) (core/mkid))
...
(defn- chat-completions->aisdk-chunks-xf
  ...
  (fn [rf]
    (let [text-id      (atom nil)
      ...
      (fn
        ([result]
         ;; Flush any collected tool calls even if the finish_reason was not seen
         ;; (defensive; normally finish_reason=tool_calls is emitted).
         (let [flush-tool (fn [acc idx {:keys [id name args]}]
                            (let [tid (or (get @index->id idx) id (core/mkid))]
                              (-> acc
                                  ...
                                  (rf {:type           :tool-input-available
                                       :toolCallId     tid
                                       :toolName       name}))))
               r1         (reduce-kv flush-tool result @tools)
        ...
        ([result chunk]
         (let [choices    (:choices chunk)
           ...
           (doseq [tc tool-delta
                   :let [idx           (:index tc 0)
                         id-from-delta (:id tc)]]
             (let [known-id (get @index->id idx)
                   tid      (or known-id id-from-delta (core/mkid))
                   tname    (get-in tc [:function :name])
                   args     (get-in tc [:function :arguments])]
               (when (and id-from-delta (not= known-id id-from-delta))
                 (swap! index->id assoc idx id-from-delta))
               (swap! tools (fn [tool-map]
                              (let [current (get tool-map idx {:id tid :name nil :args "" :index idx})]
                                (assoc tool-map idx
                                       (-> current
                                           (cond-> (seq tname) (assoc :name tname))
                                           (cond-> (seq args) (update :args str args))
                                           (assoc :id tid))))))))
           ...
           (condp = finish
             ...
             "tool_calls"
             (do
               (doseq [[idx {:keys [id name args]}] @tools
                       :let [tid (or (get @index->id idx) id (core/mkid))]]
...
(defn custom-raw
  ...
  ([opts]
   ...
   (let [model (or (not-empty (:model opts)) (llm/llm-custom-provider-model))
         req   (chat-completions-request-body (assoc opts :model model))]
     (try
       (let [api-key  (not-empty (or (:api-key opts) (llm/llm-custom-provider-api-key)))
             ...
             _        (when (str/blank? api-key)
                        (throw (core/missing-api-key-ex "Custom")))
             _        (when (str/blank? base-url)
                        (throw (ex-info (tru "Custom LLM provider base URL is not configured.")
                                        {:type :llm-not-configured})))
             _        (when (str/blank? model)
                        (throw (ex-info (tru "Custom LLM provider model is not configured.")
                                        {:type :llm-not-configured})))
