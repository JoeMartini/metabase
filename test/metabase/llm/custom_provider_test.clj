...
(deftest custom-provider-settings-test
  ...
  (testing "custom provider defaults are correct"
    (mt/with-temporary-setting-values
      ...
      (is (nil? (llm.settings/llm-custom-provider-base-url)))
      (is (nil? (llm.settings/llm-custom-provider-api-key)))
      (is (nil? (llm.settings/llm-custom-provider-model))))))

;;; -------------------------------------------------- Provider Selection Tests --------------------------------------------------

(deftest custom-provider-configured-test
  (testing "custom provider is configured when enabled and has api key and base url"
    (mt/with-temporary-setting-values
      [llm-custom-provider-enabled? true
       llm-custom-provider-api-key "sk-test"
       llm-custom-provider-base-url "https://api.example.com/v1"
  ...
  (testing "custom provider is not configured when base url is missing"
    (mt/with-temporary-setting-values
      [llm-custom-provider-enabled? true
       llm-custom-provider-api-key "sk-test"
       llm-custom-provider-base-url nil
